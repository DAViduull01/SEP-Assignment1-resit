...
class ConnectorGitter(Connector):
    ...
    def __init__(self, config, opsdroid=None):
        ...
        self.gitter_base_api = self.config.get("api-base-url", GITTER_BASE_API)
        self.gitter_stream_api = self.config.get("api-stream-url", GITTER_STREAM_API)
        self.update_interval = self.config.get("update-interval", 1)
    ...
    async def connect(self):
        ...
        current_user_url = self.build_url(
            f"{self.gitter_base_api}/v1/user/me",
        ...
        message_stream_url = self.build_url(
            f"{self.gitter_stream_api}/v1/rooms",
            self.room_id,
            "chatMessages",
            access_token=self.access_token,
        )
    ...
    async def send_message(self, message):
        ...
        url = self.build_url(
            f"{self.gitter_base_api}/v1/rooms", message.target, "chatMessages"
        )
        headers = {
